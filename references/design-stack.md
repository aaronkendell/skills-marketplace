...
| `/huashu-design` | HTML-native design skill. High-fidelity prototypes, slides, animations, 20 design philosophies, 5-dimension review, MP4 export. Agent-agnostic. | [alchaincyf/huashu-design](https://github.com/alchaincyf/huashu-design) | `/plugin install huashu-design@bokendell-skills` *(upstream has `SKILL.md` at root with no plugin layout, so it installs as a personal user-scope skill rather than a marketplace plugin — `git pull` to update)* |
...
ls ~/.claude/plugins/cache/bokendell-skills/motion/*/skills/ 2>/dev/null
```
